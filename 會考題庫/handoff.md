...
| 項目 | 數量 |
|---|---|
| 題庫題目 | 485（官方歷屆 358＋翰林模擬 81＋自編生成 46）|
| 命題模板卡 | 選擇 29 張（易 9／中 10／難 10）、非選 12 張（每冊各 2、八種問法）|
| 配圖元件 | 17 種 |
| 非選評分規準 | 38 題（官方 26＋翰林 6＋自編生成 6）|
| 觀念補強 | 56 單元 336 題 |
| Python 腳本 | 會考題庫 scripts 頂層 39 支（另有 vendor）|
...
- [ ] 把 `data/questions_SIM115.json`（25 選擇＋2 非選的完整模擬卷）派給學生試作，
      補充自編完整模擬卷的校準資料；909 班先前作答已可作為非選規準校準來源
- [ ] 本輪修改未部署；回饋單新增參考等級，待實際覆核卷驗收；影片待新 PPTX 與完整渲染驗收
...
### ✅ 2026-08-31 新增翰林 113 年第1次模擬卷（HL113）＋派科資班
- 來源：`G:\我的雲端硬碟\2026數學809\複習卷\113平孚B1_B2\`（題本＋解答篇＋非選樣卷說明三份）。
  ⚠ 資料夾名寫「113平孚」，但**實際是翰林**（封面左下角「翰林試務中心」、條碼末碼 -33）；
  「平浮」是檔名裡的印刷版本標記，不是出版社。110 年是 -30、111 年是 -31。
- **題號前綴改用年度：`HL113`**。既有 HL1＝110 年、HL2＝111 年是早期命名，
  若這份叫 HL3 就跳過了 112 年、之後補卷序號會亂。**往後新增一律用 `HL<年度>`**。
  顯示名稱要同步加進 build_html.py 的 `SRCLBL`／`YRLBL`（已加「翰林模擬 113」）。
- 建置：切圖 27 張（`--prefix HL113`）→ `data/questions_HL113.json`（含答案、108課綱雙代碼、
  冊章、詳解、逐步引導、陷阱）→ 評分規準加 HL113-N1／N2（官方逐級分指引，取自解答篇＋樣卷說明）。
  答案已與解答篇標準答案表逐題核對，解法全部自行驗算。
- 派卷：`hanlin-113`，classes 只有 `kz`（科資班）。
  學生網址 https://math809-quiz.pages.dev/q/hanlin-113-kz/
- 題庫現況：**439 題**（103-115 官方 358＋HL1 27＋HL2 27＋HL113 27）。評分規準 38 題。
- 這批卷的非選主題：N1 心臟重量與體重比例（長頸鹿／獅子）、N2 直線與兩軸圍成的面積與反求點坐標。

### ✅ 2026-09-04 交接文件補完（AGENTS.md 360 → 486 行）
老師要把專案交給別的 agent，盤點後發現入口文件幾乎只寫會考題庫，補上這些：

- **新增「接手第一天：先做這五件事」**（放在文件最前面）：config 檢查 → selftest → 讀 handoff 檔尾
  → 掃硬性規則 → 確認權限邊界；並列出新 agent 最常誤判的三件事
  （卷名的班級後綴、`0` 是合法級分、Code.gs 只存檔不生效）。
- **新增 4.5 節「另外兩個子系統」**：專案根目錄的**複習簡報六冊**與**教學影片製作**，
  原本完全沒被記錄。含六冊資料夾對照（⚠ 第四冊資料夾名是「複習網站」沒有數字）、
  引擎架構、`?v=` 版本號不 bump 學生會拿到快取舊版、六冊共用引擎要一起複製、
  以及影片規格書寫死了另一台電腦路徑（`C:\Users\mathr\...`）照跑會失敗。
- **新增第 9 章「外部服務與權限」**：八個站台、GAS、試算表、Drive、OpenAI 的擁有者與
  **agent 能不能自己操作**（GAS 重新部署只有老師能做；Drive 沒有刪除接口）；
  換電腦要手動補的三個金鑰檔；整套搬給別人用的方式；每場考試的 OpenAI 花費（30 人約 NT$40~60）。
- 專案地圖補上 `複習卷/`、根目錄的 `scripts/ working/ output/`、課本 PDF 六個資料夾。
- 現況更新到 2026-09-04，補上 8/31 翰林 113 那場的實戰數據（9 人 18 份、平均 5.22/6、0 漏批）。

**驗證方式**：寫了個檢查腳本，確認文中提到的 21 個檔案路徑真的存在、章節編號連續、
文件寫的題數（485）與 `data/questions_*.json` 實際加總相符。全部通過。

⚠ 決策說明：**沒有另開新的交接檔**。專案的文件分工已明訂 `AGENTS.md` 是入口
（`CLAUDE.md` 放藍圖、`handoff.md` 放進度、Obsidian 放系統文件），再多一份會讓接手的人
不知道該讀哪個。所以是把缺的補進 AGENTS.md。


### ✅ 2026-09-05 Codex 接手：完整健檢、回饋單與影片前置檢查

**本機改動**
- `validate.py` 改為掃描所有 `questions_*.json`：485 題（447 選擇、38 非選）全部納管；新增重複 ID／欄位型別／答案／越界圖片檢查，錯誤回傳非零退出碼，UTF-8 輸出避免 Windows cp950 崩潰。
- `selftest_all.py` 納入完整題庫、評分規準、離線回歸、七支批改 CLI 的 `--help`；quick 仍檢查根目錄、六冊與題庫 JavaScript。完整建置已實跑通過，44 個 JS 檔案／區塊通過；模板仍有兩項已知警告。
- `make_feedback_pdf.py` 支援 HL113／G0813 題號自然排序、依題庫 img 找原圖；每題另起頁，避免題目與評語擠在一起。新增 `feedback_summary.py`，唯讀合併完整卷名、班級及座號相符的最新選擇題紀錄。只在登錄題目齊全、非選全部老師覆核、選擇總題數正確時顯示 85/15 加權及參考等級。零分有效；缺資料與 AI 初評不當成最終等級。
- 根目錄 `scripts/config.py` 與 `video_runtime.example.json` 管影片機器路徑；本機覆寫檔已 gitignore。移除另一台電腦使用者路徑，`generator.py q1 --check` 支援只檢查、不製片；q1/q2 的快取前置檢查通過，HyperFrames CLI 0.7.31 可離線啟動。未重新配音、未渲染新影片；新底圖仍需指定原始 PPTX。
- README 更新為 Cloudflare 現行流程；入口與影片規格同步更新。未變更 GAS、未操作學生既有成績、未部署、未 commit/push；先前本機分支與上游參照分歧，未 fetch/pull。

**命題警告審查（保留警告，不等於消除）**
- C-B1-01：最小值題且數值選項排序，A 固定屬題型結構；100 組以獨立不等式檢查唯一正解通過。另發現干擾項說明「比丁還多」並非總成立，已改為正確的「比乙多」，不改題意／答案／既有題圖。
- C-G-01 第 2 小題：100 組獨立計費驗算通過，答案 A96／B4；偏態來自干擾項池，多數較正解大。之後重設干擾項池時要審查錯誤概念與唯一正解，不能僅為湊字母而加無意義選項。
- 選擇模板 `--n 30` 通過，仍保留上述兩項警告。

**驗證與樣本**
- `tests/test_takeover.py` 離線回歸包含：新批次缺圖／代碼錯誤／重複 ID／損壞 JSON 會失敗；零分／滿分／待覆核／缺題／無選擇題／無效分數／不同卷／最新交卷／座號正規化／選擇總題數不符。
- 回饋單排版測試只用虛構學生與 mock，未查詢真實作答；檔案在根目錄 `output/pdf/回饋單_離線測試.pdf`。已有題圖屬班級教材，不公開分享。

**下一步（尚未完成）**
1. 先以老師已完成覆核的一份實際卷驗收 PDF；若重考紀錄需按交卷批次對齊，應另加批次識別，不只靠同卷最新紀錄。
2. 批改規準校準：以 909 班既有作答，按題目比較 AI／老師級分，先看差 2 級以上、零分與低信心案例；保留原分數，只產生建議錨點與錯誤類型，老師確認後才更新規準。
3. 未提供原始 PPTX，影片僅確認既有快取與前置檔案可用。正式重製需檢查 PowerPoint、語音模型及音畫同步。

### ✅ 2026-09-06 新增翰林 112 年第1次模擬卷（HL112）＋派科資班
補上翰林系列唯一斷掉的那一年（原本有 110、111、113，獨缺 112）。

- 來源：`G:\我的雲端硬碟\2026數學809\翰林112\`（題本＋解答篇＋非選樣卷說明）。
  封面確認：112 學年度、翰林試務中心、**條碼末碼 -32**，正好符合既有規律
  （110→-30、111→-31、112→-32、113→-33）。已備份到 `00_原始試題PDF/翰林112/`（PDF 被 gitignore 排除）。
- 27 題（選擇 25＋非選 2）全部建置：答案與標準答案表逐題核對、解法自行重算；
  含 108 課綱雙代碼、冊別章節、難度、詳解、逐步引導、陷阱提示。
- 評分規準加 HL112-N1／N2（官方逐級分指引原文＋各 5 條 checkpoints）。
  N1 是「手機號碼猜年齡」的代數式推導（100x＋15，只有 2022 年適用）；
  N2 是「中位數不變 → 推得小明 ≥78 分 → 平均至少 74」的統計推理。
- 派卷 `hanlin-112`，classes 僅 kz：https://math809-quiz.pages.dev/q/hanlin-112-kz/
- 題庫站已同步部署（覆核頁才吃得到 HL112 的參考答案與評分指引）。

**這次的流程驗證**：完全照 `AGENTS.md` 的「新增一份模擬卷」SOP 走，十個步驟都適用，
沒有遇到文件沒寫到的狀況。唯一與文件不同的是第 ⑦ 步：
`validate.py` 已被改成自動掃描 `questions_*.json`，**不必再手動加清單**
（`build_html.py` 的 `SRCLBL`／`YRLBL` 仍要手動加，這次加了「翰林模擬 112」兩處）。

**題庫現況**：512 題（官方 358＋翰林 108＋自編 46）、評分規準 40 題。
翰林卷四場齊全：HL1(110)、HL2(111)、HL112(112)、HL113(113)，各 27 題。

### ✅ 2026-09-07 翰林112 考試：排程自動批改＋AI 自動放行
老師當天不在現場、無法人工覆核，要求「AI 批完直接送出，讓學生先看到分數與參考解答」。

**新增兩支腳本**
- `scripts/apply_ai_grades.py`：把 AI 級分套用成「老師覆核級分」（＝覆核頁「一鍵套用全部 AI 級分」的可排程版）。
  **為什麼必要**：學生端查成績只認「老師覆核級分」，AI 批完若不套用，學生只看得到「批改中…」。
  兩個保護：①只寫老師覆核級分還是空的那些份，**不覆蓋老師親自定過的分數**；
  ②不寫「老師備註」——學生端只有在「老師覆核級分 == AI級分」時才顯示評分理由，
  兩者一致學生才看得到「為什麼得這個分數」。可用 `--min-conf 0.7` 只放行高信心的份。
- `scripts/auto_grade_watch.py`：輪詢批改（批改→產紅筆圖→套用），平行度依待批量自動調整，
  `--until HH:MM` 到點自己結束並做最後一次總放行。`--once` 可單輪測試。
  ⚠ log 一定要用 `--log`（Python 自己以 UTF-8 寫檔）；先前交給 PowerShell 管道轉存，中文全變亂碼。

**端對端測試（已完成，資料完全還原）**
拿翰林113 已覆核完的那卷測：挑一位「老師覆核＝AI 級分」的學生（座號 5）清空覆核 → 跑套用 → 值回到原樣；
同時確認座號 9 那份（AI 給 2、老師改成 3）**沒有被覆蓋**，仍是 3。寫入功能與保護機制都驗證過。

**排程（Windows 工作排程器）**
- 任務名 `MathQuizAutoGrade_HL112`，2026-09-07 14:00 觸發，跑到 15:00，執行上限 2 小時。
- log：`會考題庫/auto_grade_20260907.log`
- ⚠ 前提是**電腦當天要開機並已登入**，否則不會執行（設了 StartWhenAvailable，開機後會補跑）。
- 查狀態／手動補跑／刪除：
  ```powershell
  Get-ScheduledTask -TaskName 'MathQuizAutoGrade_HL112' | Get-ScheduledTaskInfo
  Start-ScheduledTask -TaskName 'MathQuizAutoGrade_HL112'
  Unregister-ScheduledTask -TaskName 'MathQuizAutoGrade_HL112' -Confirm:$false
  ```
  ⚠ 手動觸發會進入 `--until 15:00` 的輪詢，要停用 `Stop-ScheduledTask`（測試時踩過，它會一直跑）。
