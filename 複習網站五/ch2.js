...
(function () {
  ...
  window.DECK.push({
    ...
    title: '圓',
    color: C,
    sections: ['2-1 點、直線與圓', '2-2 圓心角、圓周角與弧'],
    slides: [
      ...
      /* ---------- 2-1 點、直線與圓 ---------- */
      {
        ...
        points: [
          ...
          '圓上兩點間的一段曲線叫<b>弧</b>；兩半徑與弧圍成<b>扇形</b>。'
        ],
        ...
        visual: (h) => {
          ...
          s += SV.seg(A[0], A[1], B[0], B[1], BLU, 2.6);   // 直徑（過 O 嗎? A,B 200/340 非直徑）
        ...
        caption: '半徑、直徑、弦、弧、扇形——圓的所有題目都由這些元素組成。',
        example: {
          q: '一圓的直徑為 14 公分，半徑是多少？',
          steps: ['\\(r=\\dfrac{d}{2}=\\dfrac{14}{2}=7\\)。'],
          ans: '半徑 7 公分'
        }
      },

      {
        ...
        points: [
          ...
          '半徑、半弦、弦心距構成直角三角形：\\(r^2=d^2+(\\tfrac{弦}{2})^2\\)。拖滑桿看弦怎麼變。'
        ],
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            const y = O[1] - d;
            const A = [O[0] - L, y], B = [O[0] + L, y], M = [O[0], y];
            let s = circle(O[0], O[1], r, '#334');
            s += SV.seg(A[0], A[1], B[0], B[1], BLU, 3.4);
            s += SV.seg(O[0], O[1], M[0], M[1], RED, 2.6);
            ...
            s += SV.dot(B[0], B[1], '#172033', 3.5) + SV.vlabel(B[0] + 6, B[1] - 6, 'B');
            s += SV.vlabel(M[0] + 6, M[1] - 6, 'M');
        ...
        example: {
          q: '半徑 13，弦心距 5，求弦長。',
          steps: ['半弦 \\(=\\sqrt{r^2-d^2}=\\sqrt{169-25}=\\sqrt{144}=12\\)。', '弦長 \\(=2\\times12=24\\)。'],
          ans: '弦長 24'
        }
      },

      {
        sec: '2-1', secName: '位置關係',
        title: '點與圓的位置關係',
        points: [
          '比較「點到圓心的距離 \\(d\\)」與「半徑 \\(r\\)」就能判斷：',
          '\\(d\\lt r\\)：點在<b>圓內</b>；　\\(d=r\\)：點在<b>圓上</b>；　\\(d\\gt r\\)：點在<b>圓外</b>。',
          '拖滑桿改變 \\(d\\)，看點跑進跑出圓。'
        ],
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            s += `<text x="170" y="285" text-anchor="middle" font-size="16" font-weight="900" fill="${col}">d ${d < r - 2 ? '<' : d > r + 2 ? '>' : '='} r ⇒ 點在${name}（r=${r}）</text>`;
...

      {
        ...
        points: [
          '比較「圓心到直線的距離 \\(d\\)」與半徑 \\(r\\)：',
          '\\(d\\gt r\\)：<b>相離</b>（0 個交點）；\\(d=r\\)：<b>相切</b>（1 個交點，此直線為<b>切線</b>）；',
          '\\(d\\lt r\\)：<b>相交</b>（2 個交點，此直線為割線）。拖滑桿看交點數變化。'
        ],
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            const lx = O[0] - d; // 垂直線 x = O.x - d（距圓心 d）
            const [name, col, cnt] = d > r + 2 ? ['相離', RED, 0] : d < r - 2 ? ['相交（割線）', GRN, 2] : ['相切（切線）', BLU, 1];
            let s = circle(O[0], O[1], r, '#334', 'rgba(124,58,237,0.05)');
            s += SV.seg(lx, 20, lx, 260, col, 3);
            s += SV.seg(O[0], O[1], lx, O[1], '#8a94a6', 2, '4 3');
            ...
            s += `<text x="200" y="288" text-anchor="middle" font-size="16" font-weight="900" fill="${col}">${name}　${cnt} 個交點</text>`;
...

      {
        ...
        title: '切線的兩個重要性質',
        points: [
          '① 切線<b>垂直</b>於過切點的半徑：\\(OT\\perp\\) 切線。',
          '② 從圓外一點 \\(P\\) 向圓作兩條切線，<b>切線長相等</b>：\\(PA=PB\\)。',
          '這兩點是解切線題的萬用工具。'
        ],
        formula: { label: '切線長相等', tex: '\\overline{PA}=\\overline{PB}' },
        visual: (h) => {
          const O = [150, 150], r = 80, P = [370, 150];
          // 切點：由 P 對圓的兩切點
          const dOP = Math.hypot(P[0] - O[0], P[1] - O[1]);
          const a = Math.acos(r / dOP); // 半張角
          const base = Math.atan2(O[1] - P[1], O[0] - P[0]);
          const T1 = [O[0] + r * Math.cos(base + Math.PI - a + Math.PI), O[1]]; // 備用
          // 用幾何：切點 = O + r*(方向)
        ...
        caption: '半徑⊥切線（紅直角）；同一外點的兩切線長相等（PA＝PB）。',
        example: {
          q: '\\(P\\) 到圓心 \\(O\\) 距離 10，圓半徑 6，求切線長 \\(PA\\)。',
          steps: ['\\(OA\\perp PA\\)，直角三角形：\\(PA=\\sqrt{OP^2-r^2}\\)。', '\\(=\\sqrt{100-36}=\\sqrt{64}=8\\)。'],
          ans: '切線長 8'
        }
      },

      {
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            const O2 = [O1[0] + d, O1[1]];
            const sum = R + r, diff = R - r;
            let name, col;
            ...
            let s = circle(O1[0], O1[1], R, '#334', 'rgba(37,99,235,0.05)');
            s += circle(O2[0], O2[1], r, '#334', 'rgba(5,150,105,0.06)');
      ...
      /* ---------- 2-2 圓心角、圓周角與弧 ---------- */
      {
        ...
        points: [
          ...
          '整個圓是 \\(360^\\circ\\)，所以半圓的弧是 \\(180^\\circ\\)。拖滑桿看角與弧一起變。'
        ],
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            s += `<text x="${SV.pt(O[0], O[1], r + 22, 60 - t / 2)[0]}" y="${SV.pt(O[0], O[1], r + 22, 60 - t / 2)[1]}" text-anchor="middle" font-size="13" font-weight="800" fill="${C}">弧 ${t}°</text>`;
...

      {
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            h.querySelector('#al').innerHTML = svg('0 0 330 300',
              s + `<text x="315" y="60" text-anchor="end" font-size="12" fill="${C}">ℓ = 2π·${rMath}·${t}/360</text>`);
          };
        ...
        example: {
          ...
          steps: ['\\(\\ell=2\\pi\\times6\\times\\dfrac{60}{360}=12\\pi\\times\\dfrac16\\)。', '\\(=2\\pi\\)。'],
          ans: '\\(2\\pi\\)'
        }
      },

      {
        ...
        points: [
          ...
          '另一好用式子：扇形面積 \\(=\\dfrac12\\times\\)弧長\\(\\times r\\)（像三角形）。'
        ],
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            const d0 = 90 - t;
            const A = SV.pt(O[0], O[1], rr, 90), B = SV.pt(O[0], O[1], rr, d0);
            ...
            h.querySelector('#sec').innerHTML = svg('0 0 330 300',
              s + `<text x="315" y="60" text-anchor="end" font-size="12" fill="${C}">A = π·${rMath}²·${t}/360</text>`);
          };
        ...
        example: {
          ...
          steps: ['\\(A=\\pi\\times6^2\\times\\dfrac{60}{360}=36\\pi\\times\\dfrac16\\)。', '\\(=6\\pi\\)。'],
          ans: '\\(6\\pi\\)'
        }
      },

      {
        ...
        points: [
          ...
          '拖滑桿移動圓周上的頂點 \\(P\\)，圓周角<b>不變</b>（永遠是圓心角一半）。'
        ],
        ...
        visual: (h) => {
          ...
          const O = [210, 175], r = 120;
          const aA = 210, aB = 330; // 弧 AB 在下方，圓心角 = 120
          ...
          const draw = () => {
            ...
            let s = circle(O[0], O[1], r, '#334');
            s += arc(O[0], O[1], r, aA, aB + 360 - 360, GRN, 6); // 弧 AB（下方短弧）
            // 圓心角
            ...
            const lo = Math.min(dPA, dPB), hi = Math.max(dPA, dPB);
            s += SV.angle(P[0], P[1], 26, lo, hi, BLU, '60°', { w: 2.6, fs: 12 });
        ...
        example: {
          q: '一段弧所對圓心角 \\(100^\\circ\\)，求它所對的圓周角。',
          steps: ['圓周角＝圓心角的一半 \\(=\\dfrac{100^\\circ}{2}\\)。'],
          ans: '\\(50^\\circ\\)'
        }
      },

      {
        ...
        points: [
          ...
          '所以它的圓周角 \\(=\\dfrac{180^\\circ}{2}=90^\\circ\\)——<b>直徑所對圓周角必為直角</b>。',
        ...
        visual: (h) => {
          ...
          const draw = () => {
            ...
            const P = SV.pt(O[0], O[1], r, p);
            let s = circle(O[0], O[1], r, '#334');
            s += SV.seg(A[0], A[1], B[0], B[1], GRN, 3); // 直徑
            s += SV.seg(P[0], P[1], A[0], A[1], BLU, 2.6) + SV.seg(P[0], P[1], B[0], B[1], BLU, 2.6);
        ...
        example: {
          ...
          steps: ['\\(\\angle APB=90^\\circ\\)（直徑所對）。', '\\(\\angle PBA=180^\\circ-90^\\circ-35^\\circ=55^\\circ\\)。'],
          ans: '\\(55^\\circ\\)'
        }
      },

      {
        ...
        title: '弦切角 = 所夾弧的一半',
        points: [
          '由一條<b>弦</b>與過端點的<b>切線</b>所夾的角，叫<span class="k">弦切角</span>。',
          '弦切角的度數 \\(=\\dfrac12\\times\\)它所夾<b>弧</b>的度數（和圓周角一樣）。',
          '所以「弦切角＝同弧的圓周角」。'
        ],
        formula: { label: '弦切角', tex: '\\angle(\\text{弦,切線})=\\tfrac12\\,\\overparen{弧}' },
        visual: (h) => {
          const O = [210, 140], r = 100;
          const T = SV.pt(O[0], O[1], r, 270); // 切點在最下方
          const Cc = SV.pt(O[0], O[1], r, 40);  // 弦另一端
          // 切線在 T 為水平線
          let s = circle(O[0], O[1], r, '#334');
          s += SV.seg(T[0] - 120, T[1], T[0] + 120, T[1], '#334', 3); // 切線
          s += SV.seg(T[0], T[1], Cc[0], Cc[1], BLU, 3); // 弦 TC
          s += arc(O[0], O[1], r, 270, 400 - 360 + 360, GRN, 5); // 夾弧 T→C（逆時針）
          s += SV.angle(T[0], T[1], 34, 0, SV.angleOf(T[0], T[1], Cc[0], Cc[1]), RED, '', { w: 2.6, fill: 1 });
          s += SV.dot(T[0], T[1], '#172033', 4) + SV.vlabel(T[0] - 6, T[1] + 20, 'T');
          s += SV.dot(Cc[0], Cc[1], '#172033', 4) + SV.vlabel(Cc[0] + 6, Cc[1], 'C');
          s += `<text x="${T[0] + 40}" y="${T[1] - 14}" font-size="12" font-weight="800" fill="${RED}">弦切角</text>`;
          s += `<text x="${SV.pt(O[0], O[1], r + 20, 340)[0]}" y="${SV.pt(O[0], O[1], r + 20, 340)[1]}" font-size="12" font-weight="800" fill="${GRN}">夾弧</text>`;
          h.innerHTML = svg('0 0 430 300', s);
        },
        caption: '弦切角（紅）＝所夾弧（綠）的一半，用法和圓周角相同。',
        example: {
          q: '弦切角所夾的弧為 \\(140^\\circ\\)，求這個弦切角。',
          steps: ['弦切角＝夾弧的一半 \\(=\\dfrac{140^\\circ}{2}\\)。'],
          ans: '\\(70^\\circ\\)'
        }
      },

      {
        ...
        title: '圓內接四邊形：對角互補',
        points: [
          '四個頂點都在同一圓上的四邊形，叫<span class="k">圓內接四邊形</span>。',
          '它的<b>對角互補</b>（相加 \\(180^\\circ\\)）：\\(\\angle A+\\angle C=\\angle B+\\angle D=180^\\circ\\)。',
          '理由：兩個對角分別對到<b>兩段合成整圓</b>的弧，圓周角相加＝\\(\\dfrac{360^\\circ}{2}\\)。'
        ],
        ...
        example: {
          ...
          steps: ['對角互補：\\(\\angle A+\\angle C=180^\\circ\\)。', '\\(\\angle C=180^\\circ-95^\\circ=85^\\circ\\)。'],
          ans: '\\(85^\\circ\\)'
        }
      }
    ]
